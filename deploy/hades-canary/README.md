...
The manual service disables the image's HTTP health check because `doctor`, `search`, and `explain` do not serve `/readyz`; this also prevents host-side unhealthy-container remediation from interrupting a long LAPSE run. The manual containers remove themselves after every command; there is no canary daemon to stop. Preserve `data/` while evaluating the canary because it contains inventory, rejection, score, hash, LAPSE-cache, and installation provenance. To abandon the canary, remove only `/opt/subsyncd-canary` after separately reviewing or removing sidecars that the canary created.
