...
type CandidateRejection struct {
	...
	// Deprecated: retained for source compatibility; writes and reads ignore expiry.
	ExpiresAt time.Time
}
...
func (r *Repository) GetCandidateRejection(ctx context.Context, lookup CandidateRejectionLookup) (CandidateRejection, bool, error) {
	query := `SELECT media_id, language, provider_id, result_id, candidate_signature, artifact_checksum, reason_code, tool_signature, media_path, media_file_id, media_size, media_mod_time_ns, rejected_at_ns, expires_at_ns FROM candidate_rejections WHERE media_id=? AND language=? AND provider_id=? AND result_id=? AND candidate_signature=? AND tool_signature=? AND media_path=? AND media_file_id=? AND media_size=? AND media_mod_time_ns=?`
	args := []any{lookup.MediaID, lookup.Language, lookup.ProviderID, lookup.ResultID, lookup.CandidateSignature, lookup.ToolSignature, lookup.MediaPath, lookup.MediaFileID, lookup.MediaSize, lookup.MediaModTimeNS}
	...
	// Legacy expiry is ignored: deterministic rejections have no time limit.
...
func applyMediaMutationTx(ctx context.Context, tx *sql.Tx, mutation MediaEventMutation) (bool, error) {
	...
	result, err := tx.ExecContext(ctx, `INSERT INTO events(event_id, event_type, instance, kind, file_id, entity_id, series_id, outcome, created_at_ns) VALUES (?, ?, ?, ?, ?, ?, ?, 'applied', ?) ON CONFLICT DO NOTHING`, mutation.EventID, mutation.Type, mutation.Ref.Instance, mutation.Ref.Kind, mutation.Ref.FileID, mutation.EntityID, mutation.SeriesID, mutation.At.UnixNano())
	if err != nil {
	...
	case "delete":
		if mutation.SeriesID > 0 {
			rows, err := tx.QueryContext(ctx, `SELECT entity_id FROM media WHERE instance=? AND kind='episode' AND series_id=? ORDER BY entity_id`, mutation.Ref.Instance, mutation.SeriesID)
			if err != nil {
				return false, fmt.Errorf("find deleted series: %w", err)
			}
			var entities []int64
			for rows.Next() {
				var id int64
				if err := rows.Scan(&id); err != nil {
					_ = rows.Close()
					return false, fmt.Errorf("read deleted series: %w", err)
				}
			...
			if err != nil || closeErr != nil {
				return false, fmt.Errorf("read deleted series: %w", errors.Join(err, closeErr))
			}
			for _, id := range entities {
				child := mutation
				child.EventID = fmt.Sprintf("%s:episode:%d", mutation.EventID, id)
				child.SeriesID = 0
				child.EntityID = id
				if _, err := applyMediaMutationTx(ctx, tx, child); err != nil {
					return false, err
				}
			}
			return true, nil
...
func validateMediaMutation(mutation MediaEventMutation) error {
	...
	case "delete":
		if mutation.SeriesID != 0 {
			if mutation.SeriesID < 0 || mutation.Ref.Kind != domain.MediaEpisode || mutation.Ref.FileID != 0 || mutation.EntityID != 0 {
				return fmt.Errorf("series deletion identity is invalid")
			}
			return nil
		}
