...
func episodeContinuation(name string, offset int) bool {
	...
	// A mark after bare E can belong to decomposed release text. A mark
	// after an episode number must not hide that explicit continuation.
	last := suffix[match[1]-1]
	markedLetter := (last == 'E' || last == 'e') && unicode.IsMark(next)
	return !isAlphaNumeric(next) && !markedLetter
}
