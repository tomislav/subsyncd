...
func (p *observedProvider) Download(ctx context.Context, candidate domain.Candidate, output io.Writer) (DownloadMetadata, error) {
	...
	if err == nil && metadata.Filename != "" {
		p.events.Log(ctx, slog.LevelDebug, "provider.download_details", "provider download metadata", slog.String("provider", p.ID()), slog.String("candidate_id", observability.SafeText(candidate.ResultID)), slog.String("filename", observability.SafeText(metadata.Filename)))
	}
	return metadata, err
}
