...
The combined review identified two deletion integration follow-ups: active-only scan enumeration, and preservation of positively established legacy deletion during migration 003 before language backfill. Infer that deletion only with at least one search row, every search terminal/deleted, and the latest retained applied lifecycle event (by insertion ID) being a delete whose timestamp covers the media update. Preserve active status for absent/pruned audit evidence, later import/rename, or newer direct updates. Test historical deletion, active/reimport controls, same-time event ordering, scan continuation, and inventory/installation rejection. Known tombstones must not receive language backfill. This tightens the earlier subsequent-deletion-only migration behavior without network access or a startup scan.
