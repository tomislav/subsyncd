...
- Only lease configured instance/language routes. Prefer filtering claim eligibility with explicit active scope rather than destroying stored jobs/history. Filtering must happen in SQLite before priority ordering and LIMIT so obsolete rows cannot starve valid jobs. Preserve active lease ownership and safe automatic re-enable behavior. Cover removed languages and instances, no configured routes, priority and concurrency limits, and re-enable retaining valid history.
- Ensure notification/reconciliation maintenance scope does not call removed instances inadvertently; do not widen behavior beyond necessary queue scope.
