...
- The reproducible host-specific Compose/config/webhook fixtures and operator evidence are in `deploy/hades-daemon-canary`. Stages 1–4 are now complete: the canary has an active scoped Radarr connection and one exact Sonarr/Titlovi/LAPSE episode workflow. Next task: observe the three-file daemon, then test a real Sonarr connection with mapped and outside-scope events before any further scope increase.
...
- Hades preflight was installed at `/opt/subsyncd-canary` on 2026-09-04. Existing Sonarr/Radarr API keys were copied in place without being displayed; independent unused webhook-validation tokens were generated. Bazarr had no configured Titlovi account, and its OpenSubtitles credentials were encrypted without a user API key, so all provider fields remain explicit `REPLACE_BEFORE_SEARCH_*` placeholders. No provider search is safe until the operator replaces them.
...
- Current Bazarr parses the field permissively with `dateutil.parser.parse`. Bazarr+ catalog provider `0.1.7` does not use the returned expiration and retains its token for the worker lifetime. `subsyncd` keeps bounded expiry caching: it accepts RFC 3339 first, then Titlovi's observed fractional local timestamp in `Europe/Zagreb`; the existing one-time 401 refresh remains the fallback for server-side expiry or clock drift.
