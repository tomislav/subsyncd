...
- Silo integration is limited to its documented native `POST /api/v1/scan` contract on the main API listener (normally port 8090), authenticated with an admin API key in `Authorization: Bearer …`. Send only the mapped media-file path for a targeted scan, reject redirects, and never persist or surface the API key.
...
- The production image is multi-stage, fixed to Go 1.27.0, Debian 13.2, and checksummed LAPSE v2.0.5 amd64/arm64 release archives. Debian 12 is incompatible with the upstream LAPSE binary's glibc requirement. It runs as UID/GID 10001 with a read-only root filesystem in Compose; configuration is read-only and only data, temporary, and media mounts are writable.
