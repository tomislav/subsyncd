...
- The workflow persists every scored candidate without download references and caps the non-hash shortlist at three. Process it lazily by descending equal-score tiers: release score is primary, all candidates tied in the current tier are analyzed, viable ties rank by LAPSE confidence/provider priority/rating/stable identity, and only the winner is synchronized unless finalization fallback is required. Never download a lower tier after a higher tier installs. It installs only an exact hash, a policy-qualified `score_bypass`, or LAPSE `solid` result. `sync.policy: always` disables score bypass. Exact-hash installations are terminal; nonexact upgrades require the configured score delta (default 10).
